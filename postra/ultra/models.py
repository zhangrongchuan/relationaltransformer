...
class Ultra(nn.Module):
    ...
    def forward(self, data, batch):
        ...
        same_head = (batch[:, :, 0] == batch[:, 0:1, 0]).all(dim=1)  # [batch_size] bool
        same_tail = (batch[:, :, 1] == batch[:, 0:1, 1]).all(dim=1)  # [batch_size] bool

        # Default value if neither is consistent (optional — set to 0, or handle with error)
        default = torch.zeros_like(batch[:, 0, 0])
        # Select conditionally
        query_heads = torch.where(same_head, batch[:, 0, 0], torch.where(same_tail, batch[:, 0, 1],default))
        ...
        if self.alpha == 0:
            ...
            entity_graph_t, relation_graph_t = self.generate_graph_t(data, query_times, self.window_size, query_heads, query_rels,same_head[0])
    ...
    def generate_graph_t(self, data, times, window_size=3, query_heads = None, query_rels =None, mask_h=True):
        # if 'Ind' in self.inductive:
        ...
        if self.inductive in [ 'SmallpediaInd','YAGOInd','ICEWS18Ind']:
            ...
            for i, (q, qt, qr) in enumerate(zip(query_heads, times, query_rels)):
                ...
                if mask_h:
                    mask = (edge_subset[0, :] == q) & (time_type_subset < qt) & (edge_type_subset == qr)
                else:
                    mask = (edge_subset[1, :] == q) & (time_type_subset < qt) & (edge_type_subset == qr)
                valid_indices = torch.nonzero(mask, as_tuple=False).squeeze()

                if valid_indices.numel() > 0:
                    time_type_subset = time_type_subset[valid_indices]
                    edge_subset = edge_subset[:, valid_indices]
                    edge_type_subset = edge_type_subset[valid_indices]
                else:
                    # Fallback: match only head and time
                    if mask_h:
                        mask = (edge_subset[0, :] == q) & (time_type_subset < qt)
                    else:
                        mask = (edge_subset[1, :] == q) & (time_type_subset < qt)
                    valid_indices = torch.nonzero(mask, as_tuple=False).squeeze()
                ...
                offset = data.num_relations // 2

                edge_type_swapped = torch.where(
                    edge_type_subset < offset,
                    edge_type_subset + offset,
                    edge_type_subset - offset
                )
...
class EntityNBFNet(BaseNBFNet):
    ...
    def bellmanford(self, data, h_index, r_index, time_index=None, separate_grad=False):
        ...
        for layer in self.layers:
            ...
            # Bellman-Ford iteration, we send the original boundary condition in addition to the updated node states
            hidden = layer(layer_input, query, boundary, data.edge_index, data.edge_type, size, edge_weight=edge_weight, time_type=data.time_type, query_time=time_index)
            if self.short_cut and hidden.shape == layer_input.shape:
                # residual connection here
                hidden = hidden + layer_input
            hiddens.append(hidden)
    ...
    def forward(self, data, relation_representations, batch, entity_graph_t=None):
        ...
        if 'nbf' in self.use_time:
            ...
            elif 'RoPE1' in self.message_func:
                pe = self.precompute_trans_pe(self.dims[0],data.num_time,time_index.device)
                time_query = pe.expand(batch.shape[0], -1, -1).to(batch.device)
                self.time_query = self.time_projection(time_query)
                for layer in self.layers:
                    layer.time = self.time_query
            elif 'RoPE2' in self.message_func:
                pass
            else:
        ...
        if 'complex' in self.use_time:
            ...
            if 'RoPE1' in self.message_func:
                pe = self.precompute_trans_pe(self.dims[0], data.num_time, time_index.device)
                feature = torch.cat([feature, pe[time_index]], dim=-1)
                freqs_cos, freqs_sin = self.precompute_freqs_cis(self.dims[0], data.num_time,device=time_index.device)
            elif 'RoPE2' in self.message_func:
                pass
            else:
