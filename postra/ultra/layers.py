...
class GeneralizedRelationalConv(MessagePassing):
    ...
    def __init__(self, input_dim, output_dim, num_relation, query_input_dim, message_func="distmult",
        ...
        if self.message_func in ['ttranse','tcomplx','tntcomplx','RoPE1']:
            if time_dependent:
                # obtain relation embeddings as a projection of the query relation
    ...
    def forward(self, input, query, boundary, edge_index, edge_type, size, edge_weight=None, time_type=None, query_time=None):
        ...
        if self.message_func in ['ttranse','tcomplx','tntcomplx','RoPE1'] and time_type is not None:
        ...
        output = self.propagate(input=input, relation=relation, boundary=boundary, edge_index=edge_index,
                                edge_type=edge_type, size=size, edge_weight=edge_weight,time=time,time_type=time_type,query_time = query_time)
        return output

    def propagate(self, edge_index, size=None, **kwargs):
        if kwargs["edge_weight"].requires_grad or self.message_func in ["rotate",'dual' ,'split' ,'distmult','ttranse','tcomplx','tntcomplx','RoPE1','RoPE2']:
    ...
    def message(self, input_j, relation, boundary, edge_type, time=None, time_type=None, query_time=None):
        ...
        if self.message_func == "transe":
            ...
            pass
        elif self.message_func == 'RoPE1':
            if isinstance(time_type, list):
                time_j = torch.stack([torch.sum(time.index_select(self.node_dim, torch.tensor(i,device=time.device)),dim=1) for i in time_type], dim=1)
            else:
                time_j = time.index_select(self.node_dim, time_type)
                time_query = time[torch.arange(time.size(0),device=time.device), query_time].unsqueeze(1)
                time_j = time_query - time_j
            x_j_re, x_j_im = input_j.chunk(2, dim=-1)
            r_j_re, r_j_im = relation_j.chunk(2, dim=-1)
            t_j_re, t_j_im = time_j.chunk(2, dim=-1)
            message_re = x_j_re*r_j_re*t_j_re - x_j_im*r_j_im*t_j_re - x_j_im*r_j_re*t_j_im - x_j_re * r_j_im*t_j_im
            ...
            #     time_j = torch.stack([torch.sum(time.index_select(self.node_dim, torch.tensor(i,device=time.device)),dim=1) for i in time_type], dim=1)
            # else:
            #     time_delta = query_time - time_type
            #     time_j = time.index_select(self.node_dim, time_delta)
            ...
            message = torch.cat([message_re, message_im], dim=-1)
            message = self.rope_relative(message, time_delta)
        else:
    ...
    def rope_relative(self, x, d):
        """
        Apply RoPE when you only know relative distance(s).

        Args:
          x: tensor of shape (..., dim), where dim is even
             Examples:
                (batch, n_candidates, dim)
                (n_candidates, dim)
                (dim,)
          d: tensor of shape (...,), same prefix as x except last dim
             Examples:
                (batch, n_candidates)
                (n_candidates,)
                () or scalar
        ...
        Returns:
          rotated x with the same shape as x
        """
        dim = x.shape[-1]
        ...
        # expand cos/sin to interleaved dims (..., dim)
