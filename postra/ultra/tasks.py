...
def compute_ranking(pred, target, mask=None, stragety='random'):
    ...
    if stragety == 'average':
        if mask is not None:
            # filtered ranking
            ranking_strict = torch.sum((pos_pred < pred) & mask, dim=-1) + 1
        else:
            # unfiltered ranking
            ranking_strict = torch.sum(pos_pred < pred, dim=-1) + 1
        ranking = (ranking_strict + ranking)/2
    elif stragety == 'best':
        if mask is not None:
            # filtered ranking
            ranking = torch.sum((pos_pred < pred) & mask, dim=-1) + 1
        ...
        ranking = torch.tensor([
            torch.randint(l.item(), h.item() + 1, (1,)).item()
            for l, h in zip(ranking_strict, ranking)
        ])
        #ranking = torch.randint_like(ranking, low=ranking, high=ranking_strict + 1)
    return ranking
