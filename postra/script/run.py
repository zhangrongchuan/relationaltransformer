...
def test(cfg, model, test_data, device, logger, filtered_data=None, return_metrics=False):
    ...
    for test_id, batch in  enumerate(test_loader):
        print('This is batch ' + test_id)
...
if __name__ == "__main__":
    ...
    if 'Ind' in cfg.dataset['class']:
        train_data, valid_data, test_data, train_sub_data = dataset[0], dataset[1], dataset[2], dataset[3]
        train_sub_data = train_sub_data.to(device)
    else:
    ...
    if 'time_type' in dataset._data.keys():
        if 'Ind' in cfg.dataset['class']:
            train_and_validate_time(cfg, model, train_sub_data, valid_data, filtered_data=val_filtered_data, device=device,
