...
def multigraph_collator_time(batch, train_graphs):
    num_graphs = len(train_graphs)
    probs = torch.tensor([graph.edge_index.shape[1] for graph in train_graphs]).float()
    probs /= probs.sum()
    graph_id = torch.multinomial(probs, 1, replacement=False).item()
    ...
    batch = torch.cat([graph.target_edge_index[:, edge_mask], graph.target_edge_type[edge_mask].unsqueeze(0), graph.target_time_type[edge_mask].unsqueeze(0)]).t()
    return graph, batch

def multigraph_collator_time_equal_probs(batch, train_graphs):
    num_graphs = len(train_graphs)
    probs = torch.tensor([1/num_graphs for graph in train_graphs]).float()
    probs /= probs.sum()
    graph_id = torch.multinomial(probs, 1, replacement=False).item()
...
# here we assume that train_data and valid_data are tuples of datasets
def train_and_validate_time(cfg, model, train_data, valid_data, filtered_data=None, batch_per_epoch=None):
    ...
    train_quadruples = torch.cat([
        torch.cat([g.target_edge_index, g.target_edge_type.unsqueeze(0), g.target_time_type.unsqueeze(0)]).t()
        for g in train_data
    ])


    sampler = torch_data.DistributedSampler(train_quadruples, world_size, rank)
    if cfg.train.sampling == 'equal':
        train_loader = torch_data.DataLoader(train_quadruples, cfg.train.batch_size, sampler=sampler,
                                             collate_fn=partial(multigraph_collator_time_equal_probs, train_graphs=train_data))
    else:
        train_loader = torch_data.DataLoader(train_quadruples, cfg.train.batch_size, sampler=sampler, collate_fn=partial(multigraph_collator_time, train_graphs=train_data))
    batch_per_epoch = batch_per_epoch or len(train_loader)
    ...
    for i in range(0, cfg.train.num_epoch, step):
        ...
        if rank == 0:
            logger.warning(separator)
            logger.warning("Evaluate on valid")
        result = test_time(cfg, model, valid_data, filtered_data=filtered_data)
        if result > best_result:
            best_result = result
            best_epoch = epoch
...
@torch.no_grad()
def test_time(cfg, model, test_data, filtered_data=None):
    world_size = util.get_world_size()
    rank = util.get_rank()
    ...
    # process sequentially
    all_metrics = []
    for test_graph, filters, train_graph in zip(test_data, filtered_data, train_data):

        test_quadruples = torch.cat([test_graph.target_edge_index, test_graph.target_edge_type.unsqueeze(0), test_graph.target_time_type.unsqueeze(0)]).t()
        ...
        for batch in test_loader:
            ...
            if filtered_data is None:
                t_mask, h_mask = tasks.strict_negative_time_mask(test_graph, batch, train_graph)
            else:
                t_mask, h_mask = tasks.strict_negative_time_mask(filters, batch, train_graph)
...
if __name__ == "__main__":
    ...
    if "fast_test" in cfg.train:
        ...
        for graph in short_valid:
            ...
            graph.target_time_type = graph.target_time_type[mask]
    ...
    filtered_data = [
        Data(
            ...
            num_nodes=trg.num_nodes, time_type= torch.cat([trg.target_time_type, valg.target_time_type, testg.target_time_type])).to(device)
        for trg, valg, testg in zip(train_data, valid_data, test_data)
    ]


    train_and_validate_time(cfg, model, train_data, valid_data if "fast_test" not in cfg.train else short_valid, filtered_data=filtered_data,
                                batch_per_epoch=cfg.train.batch_per_epoch)

    if util.get_rank() == 0:
        logger.warning(separator)
        logger.warning("Evaluate on valid")
    test_time(cfg, model, valid_data, filtered_data=filtered_data)
    if util.get_rank() == 0:
        logger.warning(separator)
        logger.warning("Evaluate on test")
    test_time(cfg, model, test_data, filtered_data=filtered_data)
