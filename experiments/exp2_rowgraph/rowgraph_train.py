...
def evaluate(net, ds, task_type, device, max_batches=None):
    net.eval()
...
def main():
    ...
    max_steps = int(sys.argv[5]) if len(sys.argv) > 5 else 20_000
    seq_len = int(sys.argv[6]) if len(sys.argv) > 6 else 2048
    d_model = int(sys.argv[7]) if len(sys.argv) > 7 else 256
    ...
    train_ds = make_ds(
        [t for t in all_tasks if t[0] != db], "train", seq_len, seed,
    ...
    if jid:  # keep this run's SLURM log next to its own results
        for log in out_dir.glob(f"*_{jid}.out"):
